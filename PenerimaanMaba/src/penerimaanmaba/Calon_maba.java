...
private boolean isLulus(int matematika, int bahasaIndonesia, int bahasaInggris) {
    ...
    private void saveHasilSeleksi(String namaMahasiswa, int matematika, int bahasaIndonesia, int bahasaInggris, String lulus) throws SQLException {
    String query = "INSERT INTO hasil_seleksi (nama_mahasiswa, matematika, bahasa_indonesia, bahasa_inggris, lulus) VALUES (?, ?, ?, ?, ?)";
    PreparedStatement ps = conn.prepareStatement(query);
    ps.setString(1, namaMahasiswa);
    ps.setInt(2, matematika);
    ps.setInt(3, bahasaIndonesia);
    ps.setInt(4, bahasaInggris);
    ps.setString(5, lulus);
    ps.executeUpdate();
}
    ...
    try (Statement stmt = conn.createStatement()) {
        ...
        while (rs.next()) {
            ...
            model.addRow(new Object[]{namaMahasiswa, matematika, bahasaIndonesia, bahasaInggris, lulus});

            // Save the selection result to the database
            saveHasilSeleksi(namaMahasiswa, matematika, bahasaIndonesia, bahasaInggris, lulus);
        }
    } catch (SQLException e) {
        e.printStackTrace();
    }
